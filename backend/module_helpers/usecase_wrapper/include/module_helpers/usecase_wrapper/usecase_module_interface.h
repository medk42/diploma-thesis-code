...
namespace aergo::module::helpers::usecase_wrapper
{
    ...
    public:
        ...
        /// @return true if command creation was successful, false otherwise
        virtual bool createCommandFromParameters(
        ...
        /// @return SUCCESS (resumed with this call or already in running state), FAIL (resuming not supported), IN_PROGRESS (resume request was accepted, check status), ID_INVALID (task_id does not exist)
        ...
        virtual message_types::Result programStatus(uint64_t task_id, message_types::ProgramStatus& out_status) = 0;

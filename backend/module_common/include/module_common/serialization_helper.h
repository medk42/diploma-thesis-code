...
namespace aergo::module
{
    namespace serialize
    {
        ...
        /// @brief Push existing channels to buffer: [u64 channel_count_][channel_count_ * [u64 producer_module_id_][u32 producer_channel_id_]]
        ...
        inline void pushExistingChannels(std::vector<ByteT>& buf, const std::vector<aergo::module::ChannelIdentifier>& channel_list)
        {
            ...
            {
                ser::push<uint64_t>(buf, channel_id.producer_module_id_);   // [u64 producer_module_id_]
                ser::push<uint32_t>(buf, channel_id.producer_channel_id_);  // [u32 producer_channel_id_]
            }
        }
    }
    ...
    namespace deserialize
    {
        ...
        /// @brief Read existing channels from buffer: [u64 channel_count_][channel_count_ * [u64 producer_module_id_][u32 producer_channel_id_]]
        inline bool readExistingChannels(des::BufferReader& reader, std::vector<aergo::module::ChannelIdentifier>& out_channel_list)
        {
            ...
            for (uint64_t channel_id = 0; channel_id < channel_count; ++channel_id)
            {
                ...
                if (!reader.read<uint64_t>(channel_identifier.producer_module_id_))
                {
                    return false; // failed to read producer_module_id_
                }
                if (!reader.read<uint32_t>(channel_identifier.producer_channel_id_))
                {
                    return false; // failed to read producer_channel_id_
                }
            }

...
namespace aergo::default_modules::usecase_pick_and_place
{
    ...
    private:
        ...
        static std::expected<void, uw::helper::ErrorInfo> deserializePose(
            const std::vector<uint8_t>& pose_data,
            aergo::module::helpers::scene_detection_helper::Pose& out_pose
        );
        ...
        // Helper function for pick detection
        struct PickDetectionResult
        {
            uint64_t box_id;
            aergo::module::helpers::pose_utils::SE3 T_box_pick; // SE3 transformation from box frame to pick frame
            cv::Vec3d box_touch_point; // Touch point on the box
        };
        
        std::expected<PickDetectionResult, uw::helper::ErrorInfo> detectPickObject(
            const aergo::module::helpers::scene_detection_helper::Pose& pick_pose,

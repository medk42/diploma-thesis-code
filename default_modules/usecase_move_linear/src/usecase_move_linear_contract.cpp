...
static constexpr ModuleInfo module_info = {
    .module_type_identifier_ = "usecase_move_linear",
    .display_name_ = "Usecase Move Linear",
    .display_description_ = "Usecase module for moving the robot from the current position to a target position in a linear path. Target position is specified as a pose in the world coordinate system using the pen's current pose.",
...
std::string usecase_name = "MOVE_L";
std::string usecase_desc = "Usecase for moving the robot from the current position to a target position in a linear path. Target position is specified as a pose in the world coordinate system using the pen's current pose.";

p_desc::ParameterList auto_parameters(std::move(std::vector<p_desc::ParameterDescription>{
    {
        .type_ = p_desc::ParameterType::CUSTOM,
        .param_name_ = "Target Pose",
        .param_desc_ = "Target pose in the world coordinate system using the pen's current pose.",
...
{
    auto module = std::make_unique<aergo::default_modules::usecase_move_linear::UsecaseMoveLinear>(data_path, core, channel_map_info, logger, module_id, &module_info, multi_program_support, pause_support, stop_support);
    if (!module->valid())
    {
        return nullptr;
    }
